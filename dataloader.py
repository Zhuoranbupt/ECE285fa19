...
class VOCDataset(td.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label=[]
        labels=[]
        for i in lbl_tree.iter(tag='size'):
            weight= float(i.find('width').text)
        ...
        box_w = (float(xmax) - float(xmin) )/weight
        box_h = (float(ymax) - float(ymin)) /height
        boxes = [label,x_center,y_center,box_w,box_h]
